...
class Calorie(db.Model):       
    ...
    fooditem=db.StringProperty()
    calconsumed=db.StringProperty()
    physclactivity=db.StringProperty()
    calburnt=db.StringProperty()
    foodqty=db.StringProperty()
    constant=db.StringProperty()
    acttme=db.StringProperty()
    runconstant=db.StringProperty()
    calory=db.StringProperty()
...
class BPHandler(webapp.RequestHandler):
    def get(self): 
        ...
        blood=BloodPressure.all().filter('user', uservalue).order('collectionDate')
    ...
    def post(self):
        ...
        if not self.session['username']:
            directory = os.path.dirname(__file__)
            blood=BloodPressure.all().filter('user', uservalue).order('collectionDate')
            template_values = {'msgLog':'Login Required','session':self.session,'hits':pagehits,'blood':blood}
            path = os.path.join(directory, os.path.join('templates','bloodpressure.html'))
            self.response.out.write(template.render(path,template_values, _DEBUG))
        else:
            ...
            if bp1 and bp2 and bpGoal and colDate:
                ...
                if blood1:
                    ...
                    tempe=Temperature.all().filter('user', uservalue)
                    blood=BloodPressure.all().filter('user', uservalue).order('collectionDate')
                    template_values = {'bpmsg':'','session':self.session,'tempe':tempe,'heart':heart,'blood':blood,'hits':pagehits}
                    path = os.path.join(directory, os.path.join('templates','bloodpressure.html'))
                    self.response.out.write(template.render(path,template_values, _DEBUG))
            else:
                directory = os.path.dirname(__file__)
...
class WalkingTracker(webapp.RequestHandler):
    def get(self): 
        ...
        walking=Walking.all().filter('user', uservalue).order('collectionDate')
    ...
    def post(self):
        ...
        if not self.session['username']:
            directory = os.path.dirname(__file__)
            walking=Walking.all().filter('user', uservalue).order('collectionDate')
            template_values = {'msgLog':'Login Required','session':self.session,'hits':pagehits,'walking':walking}
            path = os.path.join(directory, os.path.join('templates','walking.html'))
            self.response.out.write(template.render(path,template_values, _DEBUG))
        else:
            ...
            if walkingDist and wlkdate:
                ...
                if walking1:
                    ...
                    swim=Swim.all().filter('user', uservalue)
                    cycle=Cycling.all().filter('user', uservalue)
                    template_values = {'wmsg':'','cycle':cycle,'exercise':exercise,'swim':swim,'walking':walking,'hits':pagehits,'session':self.session}
                    path = os.path.join(directory, os.path.join('templates','walking.html'))
                    self.response.out.write(template.render(path,template_values, _DEBUG))
            else:
                walking=Walking.all().filter('user', uservalue).order('collectionDate')
...
class SugarTracker(webapp.RequestHandler):
    def get(self): 
        ...
        directory = os.path.dirname(__file__)
        uservalue=self.session['username']
        sugar=Sugar.all().filter('user', uservalue).order('collectionDate')
    ...
    def post(self):
        ...
        if not self.session['username']:
            directory = os.path.dirname(__file__)
            sugar=Sugar.all().filter('user', uservalue).order('collectionDate')
            template_values = {'msgLog':'Login Required','session':self.session,'hits':pagehits,'sugar':sugar}
            path = os.path.join(directory, os.path.join('templates','bloodsugar.html'))
            self.response.out.write(template.render(path,template_values, _DEBUG))
        else:
...
class HoursSlept(webapp.RequestHandler):
    def get(self): 
        ...
        directory = os.path.dirname(__file__)
        uservalue=self.session['username']
        sleep=Sleep.all().filter('user', uservalue).order('collectionDate')
    ...
    def post(self):
        ...
        if not self.session['username']:
            directory = os.path.dirname(__file__)
            sleep=Sleep.all().filter('user', uservalue).order('collectionDate')
            template_values = {'msgLog':'Login Required','session':self.session,'hits':pagehits,'sleep':sleep}
            path = os.path.join(directory, os.path.join('templates','sleep.html'))
            self.response.out.write(template.render(path,template_values, _DEBUG))
        else:
...
class BodyTemperature(webapp.RequestHandler):
    def get(self): 
        ...
        directory = os.path.dirname(__file__)
        uservalue=self.session['username']
        tempe=Temperature.all().filter('user', uservalue).order('collectionDate')
    ...
    def post(self):
        ...
        if not self.session['username']:
            directory = os.path.dirname(__file__)
            tempe=Temperature.all().filter('user', uservalue).order('collectionDate')
            template_values = {'msgLog':'Login Required','session':self.session,'hits':pagehits,'tempe':tempe}
            path = os.path.join(directory, os.path.join('templates','bodytemperature.html'))
            self.response.out.write(template.render(path,template_values, _DEBUG))
        else:
...
class HeartRate(webapp.RequestHandler):
    def get(self): 
        ...
        directory = os.path.dirname(__file__)
        uservalue=self.session['username']
        heart=Heart.all().filter('user', uservalue).order('collectionDate')
        tempe=Temperature.all().filter('user', uservalue)
        blood=BloodPressure.all().filter('user', uservalue)
    ...
    def post(self):
        ...
        if not self.session['username']:
            directory = os.path.dirname(__file__)
            heart=Heart.all().filter('user', uservalue).order('collectionDate')
            template_values = {'msgLog':'Login Required','session':self.session,'hits':pagehits,'heart':heart}
            path = os.path.join(directory, os.path.join('templates','heartrate.html'))
            self.response.out.write(template.render(path,template_values, _DEBUG))
        else:
...
class Profile(webapp.RequestHandler):
    ...
    def post(self):
        ...
        for a in userdata:
            ...
            template_values = {'profilemsg':'Profile Updated','userdata':userdataN,'session':self.session,'hits':pagehits}
            path = os.path.join(directory, os.path.join('templates','profile.html'))
            self.response.out.write(template.render(path,template_values, _DEBUG)) 
...
class WeightBmi(webapp.RequestHandler):
    def get(self): 
        ...
        directory = os.path.dirname(__file__)
        uservalue=self.session['username']
        wellness=Wellness.all().filter('user', uservalue).order('collectionDate')
        template_values = {'msg':'Please Login to Enter data','wellness':wellness,'session':self.session,'hits':pagehits}
    ...
    def post(self):
        ...
        if not self.session['username']:
            directory = os.path.dirname(__file__)
            wellness=Wellness.all().filter('user', uservalue).order('collectionDate')
            template_values = {'msgLog':'Login Required','session':self.session,'hits':pagehits,'wellness':wellness}
            path = os.path.join(directory, os.path.join('templates','weightbmi.html'))
            self.response.out.write(template.render(path,template_values, _DEBUG))
        else:
            ...
            if bmiDate and bmiWeight and bmi and bmiHeight:
                ...
                if wellness1:
                    ...
                    bmiupdate.put()
                    directory = os.path.dirname(__file__)
                    template_values = {'bmimsg':'','wellness':wellness,'session':self.session}
                    path = os.path.join(directory, os.path.join('templates','weightbmi.html'))
                    self.response.out.write(template.render(path,template_values, _DEBUG))
            else:
                directory = os.path.dirname(__file__)
...
class Exerciseminutes(webapp.RequestHandler):
    def get(self): 
        ...
        directory = os.path.dirname(__file__)
        uservalue=self.session['username']
        exercise=Exercise.all().filter('user', uservalue).order('collectionDate')
        #walking=Walking.all().filter('user', uservalue)
        #swim=Swim.all().filter('user', uservalue)
        #cycle=Cycling.all().filter('user', uservalue)
        ...
        if not self.session['username']:
            ...
            if wrktme and exdate:
                ...
                if exercise1:
                    ...
                    directory = os.path.dirname(__file__)
                    exercise=Exercise.all().filter('user', uservalue).order('collectionDate')
                    template_values = {'emsg':'','exercise':exercise,'hits':pagehits,'session':self.session}
                    path = os.path.join(directory, os.path.join('templates','exercise.html'))
                    self.response.out.write(template.render(path,template_values, _DEBUG))
            else:
                exercise=Exercise.all().filter('user', uservalue).order('collectionDate')
...
class Swimmingexercise(webapp.RequestHandler):
    def get(self): 
        ...
        exercise=Exercise.all().filter('user', uservalue)
        walking=Walking.all().filter('user', uservalue)
        swim=Swim.all().filter('user', uservalue).order('collectionDate')
        cycle=Cycling.all().filter('user', uservalue)
        template_values = {'msg':'','cycle':cycle,'exercise':exercise,'swim':swim,'walking':walking,'hits':pagehits,'session':self.session}
    ...
    def post(self):
            ...
            if not self.session['username']:
                directory = os.path.dirname(__file__)
                swim=Swim.all().filter('user', uservalue).order('collectionDate')
                template_values = {'msgLog':'Login Required','session':self.session,'hits':pagehits,'swim':swim}
                path = os.path.join(directory, os.path.join('templates','swimming.html'))
                self.response.out.write(template.render(path,template_values, _DEBUG))
            else:
                ...
                if swimDist and swimTime:
                    ...
                    if swim1:
                        ...
                        swim=Swim.all().filter('user', uservalue).order('collectionDate')
                        cycle=Cycling.all().filter('user', uservalue)
                        template_values = {'cmsg':'','session':self.session,'cycle':cycle,'exercise':exercise,'walking':walking,'swim':swim,'hits':pagehits}
                        path = os.path.join(directory, os.path.join('templates','swimming.html'))
                        self.response.out.write(template.render(path,template_values, _DEBUG))
                else:
                    directory = os.path.dirname(__file__)
...
class Cyclingexercise(webapp.RequestHandler):
    def get(self): 
        ...
        walking=Walking.all().filter('user', uservalue)
        swim=Swim.all().filter('user', uservalue)
        cycle=Cycling.all().filter('user', uservalue).order('collectionDate')
        template_values = {'msg':'','cycle':cycle,'exercise':exercise,'swim':swim,'walking':walking,'hits':pagehits,'session':self.session}
    ...
    def post(self):
        ...
        if not self.session['username']:
            directory = os.path.dirname(__file__)
            cycle=Cycling.all().filter('user', uservalue).order('collectionDate')
            template_values = {'msgLog':'Login Required','session':self.session,'hits':pagehits,'cycle':cycle}
            path = os.path.join(directory, os.path.join('templates','cycling.html'))
            self.response.out.write(template.render(path,template_values, _DEBUG))
        else:
...
class Calories(webapp.RequestHandler):
    def get(self):
        self.session = Session()
        hit=Counter.all()
        for hi in hit:
        ...
        uservalue=self.session['username'] 
        directory = os.path.dirname(__file__)
        #fooditem=self.request.get('fooditem')
        calorie=Calorie.all().filter('user', uservalue)
        template_values = {'msg':' ','session':self.session,'hits':pagehits,'calorie':calorie}
        self.session['type']='admin'
        path = os.path.join(directory, os.path.join('templates','calories.html'))
        self.response.out.write(template.render(path,template_values, _DEBUG))      
    ...
    def post(self):
        ...
        pagehits=hit.fetch(1)
        uservalue=self.session['username']
        foodqty=self.request.get('qty')
        constant=self.request.get('cnst')
        calconsumed=self.request.get('con')
        physclactivity=self.request.get('run')
        runconstant=self.request.get('runcnst')
        calburnt=self.request.get('rest')
        calory=self.request.get('')
        collectionDate=self.request.get('caldate')
        activity=self.request.get('activity')
        fooditem=self.request.get('calory')
        calorie=Calorie.all().filter('user', uservalue)
        calupdate=Calorie(calorie=calorie,foodqty=foodqty,constant=constant,calconsumed=calconsumed,physclactivity=physclactivity,calburnt=calburnt,calory=calory,activity=activity,fooditem=fooditem,collectionDate=collectionDate,user=self.session['username'])
        calupdate.put()
        template_values = {'msg':' ','session':self.session,'hits':pagehits,'foodqty':foodqty,'calconsumed':calconsumed,'physclactivity':physclactivity,'calburnt':calburnt,'constant':constant,'runconstant':runconstant,'calory':calory,'calorie':calorie}
        path = os.path.join(directory, os.path.join('templates','calories.html'))
        self.response.out.write(template.render(path,template_values, _DEBUG)) 
